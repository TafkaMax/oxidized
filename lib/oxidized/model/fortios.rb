class FortiOS < Oxidized::Model
  ...
  cmd :secret do |cfg|
    ...
    cfg.gsub! /(set private-key|set privatekey ).*?-+END (ENCRYPTED|RSA|OPENSSH) PRIVATE KEY-+\n?"$/m, '\\1 <configuration removed>'
    cfg.gsub! /(set ca|set certificate )"-+BEGIN.*?-+END CERTIFICATE-+"$/m, '\\1 <configuration removed>'
